...
def shuttle(
    ...
    for pz in graph.pzs:
        ...
        if len(ions_at_pz) == 2:
            ...
            # TODO ion1 swap also necessary? # new TODO
        ...
        elif len(ions_at_pz) == 1:
            ion1 = ions_at_pz[0]
            if ion1 in gate_info_list[pz.name] and len(gate_info_list[pz.name]) == 1:
                graph.stop_moves.append(ion1)
...
def main(graph: Graph, sequence: list[int] | None, cycle_or_paths: str, record_path: str | None = None) -> int:
    ...
    graph.in_process = []
    graph.locked_gates = cast("dict[int, str]", {})
    while timestep < max_timesteps:
        ...
        try:
            graph.executed_gates_next = processed_execs
        except Exception:
            graph.executed_gates_next = []
