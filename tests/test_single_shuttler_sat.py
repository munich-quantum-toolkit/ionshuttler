...
class TestCreateGraph:
    ...
    def test_create_graph_basic(self):
        ...
        pz_nodes = [node for node in graph.nodes() 
                   if graph.nodes[node].get('node_type') == 'processing_zone_node']
        assert len(pz_nodes) == 1
        
        # Check that junction nodes exist
        junction_nodes = [node for node in graph.nodes() 
                         if graph.nodes[node].get('node_type') == 'junction_node']
...
class TestIdcDictionary:
    ...
    def test_create_idc_dictionary(self):
        ...
        idc_dict = create_idc_dicitonary(graph)
        
        assert isinstance(idc_dict, dict)
        assert len(idc_dict) == 2
        assert all(isinstance(key, int) for key in idc_dict.keys())
